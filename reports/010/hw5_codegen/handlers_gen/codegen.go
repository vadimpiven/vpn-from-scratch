...
func (srv *{{$name}}) handler{{.Name}}(w http.ResponseWriter, r *http.Request) {
    ...
    {{end}}if err := r.ParseForm(); errorOccurred(err, w) {
        return
    }
    ...
    if err = checkMin{{if .IsString}}String{{else}}Int{{end}}("{{.ParamName}}", {{.Name}}, {{.Min.Edge}}); errorOccurred(err, w) {
        return
    }{{end}}{{if .Max.IsSet}}
    if err = checkMax{{if .IsString}}String{{else}}Int{{end}}("{{.ParamName}}", {{.Name}}, {{.Max.Edge}}); errorOccurred(err, w) {
        return
    }{{end}}{{end}}
...
type (
    ...
    TemplateArgs struct {
        Package string
        ApiList map[string][]Method
    }
)

func main() {
    src, err := parser.ParseFile(token.NewFileSet(), os.Args[1], nil, parser.ParseComments); check(err)
    args := TemplateArgs{
    ...
    for _, decl := range src.Decls {
        ...
        for _, comment := range currFunc.Doc.List {
            if strings.HasPrefix(comment.Text, "// apigen:api ") {
                ...
                js := &MethodJSON{}
                err := json.Unmarshal([]byte(strings.TrimPrefix(comment.Text, "// apigen:api ")), js); check(err)
    ...
    tmpl, err := template.New("generated file template").Parse(Template); check(err)
    err = tmpl.Execute(w, args); check(err)
}
