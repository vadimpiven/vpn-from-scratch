...
func (srv *MyApi) handlerCreate(w http.ResponseWriter, r *http.Request) {
	if err := r.ParseForm(); !isPost(w, r) || !isAuthorized(w, r) || errorOccurred(err, w) {
		return
	}
	data := r.PostForm
	Login, err := getString("login", data, true)
	if errorOccurred(err, w) ||
	   errorOccurred(checkMinString("login", Login, 10), w) {
		return
	}
	...
	if errorOccurred(err, w) ||
	   errorOccurred(checkMinInt("age", Age, 0), w) ||
	   errorOccurred(checkMaxInt("age", Age, 128), w) {
		return
	}
...
func (srv *OtherApi) handlerCreate(w http.ResponseWriter, r *http.Request) {
	...
	if errorOccurred(err, w) ||
	   errorOccurred(checkMinString("username", Username, 3), w) {
		return
	}
	...
	if errorOccurred(err, w) ||
	   errorOccurred(checkMinInt("level", Level, 1), w) ||
	   errorOccurred(checkMaxInt("level", Level, 50), w) {
		return
	}
